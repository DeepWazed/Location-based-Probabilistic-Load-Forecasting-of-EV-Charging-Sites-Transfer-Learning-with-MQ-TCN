...
class taskTCN(nn.Module):
    def __init__(self, input_size, embedding_dims, num_channels, kernel_size, dropout, output_size):
        super(taskTCN, self).__init__()
        self.embedding = nn.ModuleList([nn.Embedding(num_categories, emb_dim) for num_categories, emb_dim in embedding_dims])

        self.tcn = nn.Sequential(
            *[TemporalBlock(
                num_channels[i-1] if i > 0 else input_size + sum([emb_dim for _, emb_dim in embedding_dims]),
                num_channels[i],

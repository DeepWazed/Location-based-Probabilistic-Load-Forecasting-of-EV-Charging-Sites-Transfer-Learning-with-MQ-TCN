...
class FineTunedTimeSeriesModel(nn.Module):
    def __init__(self, base_model, freeze_embedding, num_frozen_tcn_layers, input_size, embedding_dims, num_channels, kernel_size, dropout, output_size, quantiles):
        ...
        # self.num_adapter = nn.Linear(41, 50)  
        self.num_adapter = nn.Linear(40, 50)  

        self.linear_transfer_to_base = nn.ModuleList([
            nn.Linear(emb_dim, base_emb.weight.shape[0]) 
            for emb_dim, base_emb in zip([emb.embedding_dim for emb in self.transfer_embedding], base_model.embedding)
        ])
